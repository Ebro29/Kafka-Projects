...
public class ClickListerService {
    ...
    public void process(KStream<String, UserClick> input) { //read all user clicks and process them
        /*
        peek() will print incoming records
        groupBy() groups by the current key
        windowedBy() sets session windows to every 5 minutes
        count the sessions, convert it to KStream, then print with the for loop
         */
